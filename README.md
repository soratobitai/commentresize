# ニコ生コメントサイズ調整

## description
ニコ生のコメント欄の文字サイズを調整する機能を追加します。  
流れるコメントではなく、右側のコメント欄のコメントサイズを調整できます。  
デフォルトのサイズから50%～300%の間で設定できます。 
  
■ オプション  
長いコメントを折り返して全文表示〈弾幕コメは除外〉

## 【使い方】
- ニコ生のプレイヤー設定アイコンの右側にある「Aa」というアイコンをクリックすると設定画面が開きます。  
- コメントサイズとコメント番号のサイズを調整できます。  
- スライダーを左右に動かすことでリアルタイムに変更が適応されます。  
- スライダーの上にカーソルを乗せた状態でマウスホイールを回すことで１メモリずつ動かすこともできます。  
- スライダーをダブルクリックでデフォルトの位置（100%）に戻ります。

## 更新履歴
1.2.13　折り返しのあるコメント（3〜4行）の直後に、最新コメントまでスクロールが届かず止まる問題を修正  
1.2.12　一部環境で起きた画面のチカチカ（余白⇄補正のループ）と、長い折り返しコメントでスクロール・コメントが固まる問題を修正  
1.2.11　余白の根本対策（ニコ生の仮想スクロール行高さを実際の行高さに同期）＋エモーション画面から戻った時に最新が見えない問題を修正  
1.2.10　「最新コメントに戻る」を押した時に最新コメントの下に余白が出る問題を修正  
1.2.9　コメント番号順の並べ替えを再実装（スクロール停止時のみ動作し、クラッシュしない方式に変更）  
1.2.8　コメントが活発な時に激しくスクロールするとクラッシュする問題を修正（1.2.7の並べ替え処理を一旦撤去）  
1.2.7　古いコメントを遡った際にコメント番号順に並ばない問題を修正  
1.2.6　手動で上にスクロールしても最新コメントに引き戻される問題を修正  
1.2.5　軽微な修正  
1.2.4　初期化のタイミングを調整  
1.2.3　バグ修正  
1.2.2　コメント欄のレイアウトが崩れる問題を修正  
1.2.1　設定画面のデザインと表示位置を変更、他微修正  
1.2.0　設定画面に機能の有効無効ボタンを追加、他バグ修正  
1.1.9　軽微な調整  
1.1.8　パフォーマンスの改善、追加バグ修正  
1.1.7　コメント欄がちらつくバグを修正  
1.1.5　軽微な修正  
1.1.4　コメント折り返しオプション選択時、見やすく改善。他バグ修正  
1.1.3　バグ修正  
1.1.2　バグ修正  
1.1.1　なるべく弾幕コメは折り返さないように修正、他バグ修正  
1.1.0　長いコメントを折り返して全文表示するオプションを追加しました。  
1.0.0　リリース

## Author
soratobitai

## License
[MIT license](https://en.wikipedia.org/wiki/MIT_License).